...
Use this page only if `install_packages.py` reports an error or `check_packages.py` still says that pandas is not ready.
...
4. If VS Code asks which dependency file to install, select `class-09/requirements.txt`. Otherwise, wait for the environment to finish and run `install_packages.py`.
